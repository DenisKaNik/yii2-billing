...
if options['github_token'].nil? || options['github_token'].to_s.length != 40
  puts "You must place REAL GitHub token into configuration:\n/yii2-billing/vagrant/config/vagrant-local.yml"
  exit
end
...
# copy env from example if local env not exists
FileUtils.cp env[:example], env[:local] unless File.exist?(env[:local])
